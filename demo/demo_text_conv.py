...
class DemoTextConv:
    ...
    def demo(self, model: ModelsTextConversation):
        ...
        if model :
            new_user_input_ids = model.tokenizer.encode("Hello ! " + model.tokenizer.eos_token,
                                                                   return_tensors='pt')
            ...
            print("Chatbot: {}".format(
                model.tokenizer.decode(chat_history_ids[:, bot_input_ids.shape[-1]:][0],
                                                  skip_special_tokens=True)))
            print("User : How are you ?")
            new_user_input_ids = model.tokenizer.encode("How are you ? " + model.tokenizer.eos_token,
                                                                   return_tensors='pt')
            # append the new user input tokens to the chat history
            bot_input_ids = torch.cat([model.chat_history_ids, new_user_input_ids],
                                      dim=-1) if model.conversation_step > 0 else new_user_input_ids
